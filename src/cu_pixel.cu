...
__global__ void cart_to_polar_kernel(int img_w, int16_t *hori, int16_t *vert, uint16_t *rad, float *phi)
{
    const int x = blockIdx.x * 8 + threadIdx.x;
    const int y = blockIdx.y * 8 + threadIdx.y;
    const int p = img_w * y + x;
    ...
    phi[p] = atan2((double)grad_x, (double)grad_y);
}
...
extern "C" void cu_cart_to_polar(int img_w, int img_h, void *gm_hori, void *gm_vert, void *gm_rad, void *gm_phi)
{
    ...
    cart_to_polar_kernel<<<blocks, threads>>>(img_w, (int16_t *)gm_hori, (int16_t *)gm_hori, (uint16_t *)gm_rad, (float *)gm_phi);
}
